...
public class ChatinClient {
    ...
    public static void main(String[] args) {
        try {
            ...
            // Crear thread per llegir missatges del servidor
            Thread serverReaderThread = new Thread(() -> {
                try {
                    String serverInput;
                    while ((serverInput = serverIn.readUTF()) != null) {
                        if (!serverInput.isEmpty()) {
                            System.out.println("Servidor: " + serverInput);
                        }
                        if (serverInput.equalsIgnoreCase("FI")) {
                            break;
                        }
                    }
                    System.out.println("El servidor ha finalitzat la conversa.");
                    System.exit(0); // Acabar el client després que el servidor finalitzi la conversa
                } catch (IOException e) {
